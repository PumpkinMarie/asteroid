...
Asteroids::Asteroids(SDL_Window* window, float radiusMax,SDL_FPoint center) : window_(window) {
    float speedx = rand()%3;
    float speedy = rand()%3;
    //on vérifie que l'astéroide n'est pas à l'arrêt
    if (speedx-1 == 0 && speedy-1 == 0)
...
void Asteroids::draw() {
    ...
    for (int i = 0; i < nb_points_; i++) {
        ...
        SDL_RenderDrawLineF(
            renderer, data_copie[i % nb_points_].x, data_copie[i % nb_points_].y, data_copie[j % nb_points_].x, data_copie[j % nb_points_].y);
    }
...
/*void Asteroids::changeData() {
    for (int i=0;i<nb_points_;i++){
        ...
        data_copie[i] = {float(cos(i*2*M_PI/nb_points_))*radius_+center_copie.x,float(sin(i*2*M_PI/nb_points_))*radius_+center_copie.y};
    }
}*/
...
void Asteroids::changeData() {
    for (int i=0;i<nb_points_;i++){
        ...
        data_copie[i] = {float(cos(i*2*M_PI/nb_points_))*variations_[i]+center_copie.x,float(sin(i*2*M_PI/nb_points_))*variations_[i]+center_copie.y};
    }
}
